...
如果你靠硬实力无法打进速通，根据上文`跳级`章节的图像可以得到以下一系列有用的二级结论：
    故意等到掉级后再立刻断b，缓冲条内也没有垃圾，那么只需60蓄力攻击就能刚好到进速通的门口，趁5秒保护马上补满就行  
    考虑到时机误差和抵消垃圾行导致少量经验丢失，通常建议攒到70可以保证稳进  
    如果完全不考虑上述时机问题，就需要80甚至90才能有较高成功率
