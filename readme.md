...
1. 全消时 **+3**  
1. 【机心专杀】记录S/Z spin消行时用的是S还是Z，如果最近之前六个中有五个和此次相同则 **+2**  
1. 【机心专杀】如果I块竖着放在场地边两列没有消行，那么检查同侧边两列的另一列相邻四格，如果都是I块的青色格子则 **+3**  
1. 消除灰色格子（垃圾行）时立刻 **减为0**  
1. 攻击缓冲释放时，场地里每冒出一行垃圾 **-3**  
1. I块spin消行时 **-2**  
1. 消四时 **-3** ，记录列数，如果和前两次（不重复记录，只留新的）都不一样则再 **-4**  
2. S/Z spin消行时和I块消四一样，只是换成旋转中心列数（独立的表），都不一样时 **-2**  
3. J/L spin消行时和S/Z spin一样  
4. 每30秒没有做减少`连续抵消`的动作时 **+5**  
5. 每75块没有做减少`连续抵消`的动作时 **+5**  
