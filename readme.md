...
每个Mod都有一个超级强化版本（除了Duo(双人)），需要带着对应的Mod累计爬3万米解锁（开了多个可以同时累计）
...
### 专家+ （塔罗牌：（皇帝逆位）暴君 The Tyrant）

在原有的很难爬高的基础上，  
新增下坠加速度机制，更难爬高，打出攻击往上跳一点后如果一会没动就会开始下坠直到落到当前楼层底部（加速度和终端速度和楼层有关）

### 无暂存+ （塔罗牌：（节制逆位）禁欲 Asceticism）

在原有的无暂存基础上，  
1预览，非7-Bag

### 混乱垃圾行+ （塔罗牌：（命运之轮逆位）铅骰 Loaded Dice）
...
初始场地变成固定的六个圈的图案：
    ...
    .XXX..XXX.
    ..........
    这玩意摞三层，互相和墙都不接触，和麻将的六饼/六筒一样

### 高重力+ （塔罗牌：（塔逆位）自由落体 Freefall）

从一开局起就20G

### 不稳定垃圾行+ （塔罗牌：（力量逆位）顽抗 Last Stand）

三倍受击（没有三倍抵消）  
14高场地  
垃圾行孔洞位置有两个next

### 双洞垃圾行+ （塔罗牌：（恶魔逆位）天谴 Damnation）

初始场地变成10行棋盘垃圾行  
垃圾行变成每行随机3~4个灰格的混沌垃圾行

### 隐形+ （塔罗牌：（隐士逆位）放逐 The Exile）

在原有的隐形基础上，  
场地不再闪烁

### All-Spin+ （塔罗牌：（魔法师逆位）邪术师 The Warlock）
