...
> 距离下一层还有2m内时，消行就可以立刻+3m来进入下一层，此规则仅在
...
有一个`受击保护`变量，受到攻击时，增加行数等量的值（【力量+】的三倍不计），直到**18**封顶
...
| 【混乱+】 | +100% |
| 【全旋+】 | +30% |
| `全散`效果（专家+到达6分钟） | =100%（前面的机制允许超过100%，但此效果会强制改为100%） |
| `受击保护`（最终生成垃圾行时计算） | 每1点`受击保护`让Y减少3.75%和让X减少1.5% |
...
每个Mod都有对应的强化版本（除了双人），需要带着对应的Mod累计爬3万米解锁（开了多个可以同时累计）
