...
如果`受击缓冲`内有值，每固定间隔（取自上边这个表）将`受击缓冲`内0.25的值转移回`受击权重`

## 攻击、Allspin、B2B

普通消1、2、3行攻击为1、1、2 （注意消1有1的攻击，其实效率很高）

消4攻击为4，属于`特殊消除`

此模式使用Allspin规则，所有的块的spin都是`特殊消除` （其中T使用三角判定，其他块使用近似不可移动判定）

spin的基础伤害对于T是消行数×2，其他块为消行数×1，属于`特殊消除`

全消攻击为4，记为`两个特殊消除`

注：目前版本下如果pc时的消除不是spin，左上角的b2b计数会不更新，再打一个spin才能看到
