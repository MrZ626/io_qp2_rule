...
全消时 **+5**  
每30秒没吃进过垃圾行时 **+5** ，该计时器归零  
消除灰色格子（垃圾行）时立刻 **变为0**  
红条释放为垃圾行时，场地底部每冒出一行 **-3** （最低为0）  
I块spin消行时 **-2**  
消四时 **-3** ，记录列数，如果和前两次（不重复记录，只留新的）都不一样则再 **-4**  
SZ/JL块spin消行时和I块消四一样，只是换成旋转中心列数，都不一样时 **-2** （SZ和JL分别共用一个列表）

当`连续抵消`达到一定值后，bag7序列会被修改，向每包添加额外方块：
...
> 此机制基本可以认为是由于定式[机械之心](https://bilibili.com/opus/997806608970940469)的出现而添加的
...
1. 计算每一列的`难挖度`
    1. 如果该列是最高列（之一）且没有空格，该列`难挖度`为0
    1. 找到该列最低的空格，找到场地最高点的高度
    1. `难挖度 = 空格到场地顶的高度差 + 5*空格到最浅垃圾行洞的水平距离`
1. 根据`难挖度`给十列进行排序，分高的在前（相同的值随机排）
1. 根据`垃圾难度`计算概率分布，公式为`weight[i] = 10 + favor * (1 - i / 4.5)`，负数视为0
...
- BLIGHTED：消除垃圾行之后获得，持有时下一次消除攻击*1.75并+1，触发后消失
- 场地内固定有4行垃圾，挖了瞬间出
- 启用`垃圾行保护`（呃…不是固定有4行吗）
