...
`推进器等级`（下称`rank`，代码中命名如此）决定了爬塔的速度，所有增加高度的行为都受到这个倍率的加成。
...
| 发送攻击 | 每行 1m 和 1.05xp |
| 抵消垃圾行 | 每行 0.5m，（非专家模式）0.55xp |
| 消行 | 一行1m 两行及以上2m，（非专家模式）一行1.05xp 两行及以上2.05xp |
| 击杀 | 15m |

> 注意所有的高度增加都受 `rank` 影响，都要 `*rank/4`，例如开局时倍率为×0.25，每4秒增加1m

推进器升级所需xp为 `4*rank`。当xp大于需求点数时触发升级并扣除需求点数，同时推进器xp会 `锁定5秒` 不流失（详见两节后的xp流失段落）
...
推进器xp会随时间流失，每帧减少 `e*rank*(rank+1)/3600` 点。
...
每次降级时，前文提到的推进器xp锁定5秒会减少1秒，反复降级升级会更挣扎，不过至少保留1秒。重新达到高一级所需升级xp的一半时重置到5秒
