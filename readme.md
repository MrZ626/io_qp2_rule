...
ASC-DX的踢墙表（顺时针和0←→2的180°用这个表，逆时针和1←→3的180°使用左右镜像对称）：
```
...
> Only a fool would commit heresy this blatantly.  
> 只有蠢人才会如此公然行亵渎之事。

2026年愚人节活动限定，一周内免费游玩

- 24高场地
- 每17块中会有3块被转化为变异块，变异会改变方块的形状并让名字变成`[原名]-[楼层数]`，每种块每个楼层能变异成什么形状单独打表
- 每上一层楼后会有1包的安全时间无变异块
