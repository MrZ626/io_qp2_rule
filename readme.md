...
| 动作 | 效果 |
| - | - |
| 随时间每帧增加 | 每秒1m，但在距离下一层6m~1m时此速率会从×1逐渐变为×0，需要消行或者击杀才能进入下一层 |
| 击杀 | 15m （“专家+”中削弱为8m） |
| 发送攻击 | 每行 1m 和 1.05xp |
| 抵消垃圾行（非专家） | 0.55xp |
| 消行（非专家） | 一行1.05xp 两行及以上2.05xp |
...
另外qp2中有“0连击普通消一+1攻击”，这让散消一也有1的攻击效率了，特定情况可以考虑利用
...
从第二个`特殊消除`开始会累计B2B计数，同时提供1的额外伤害

所有伤害保留小数计算，最后小数部分按照数值概率向上取整，例如1.2最后有20%概率变成2，80%概率变成1

### 蓄力攻击（Surge Attack）

断B2B时会产生一个超大攻击，伤害为B2B数-3，或者说连续特殊消除数-4（TL中略有不同，不-3，几B2B就打几）
...
拆分方式：把攻击拆成若干个4和剩余部分（若有力量Mod则改成8）
...
具体来说，qp2的默认情况中Y=2.5*X，也就是打入垃圾行的间断点处有更大的概率（2.5倍）不在同一列

`专家`和`混乱垃圾行`Mod和`专家+`的疲劳效果都会增加混乱度  
...
> 此Mod可以加快前期爬楼进度，但后期的威胁又会太大，最终效果是否正面取决于要打什么项目
...
> 如果你想尽快解锁所有强化Mod的话建议同时开几个，以下给出一个参考方案（假设你可以畅玩所有的单Mod且能拿到几个f10）：  
> `重力+混乱+双洞+AllSpin` 这四个一起开然后靠allpin猛猛输出，尽量不要让垃圾行进来，刷满3万米。如果你能用循环定式无脑玩Blitz模式不怕`隐形`就可以带，否则之后要单独打  
> 然后打 `隐形+力量` 和 `无暂存+力量`，注意尽量多吃垃圾然后用消四挖开会爬得更快（因为抵消垃圾行会亏），同时尝试带和不带`专家`计算一下总效率（高度/耗时，带专家就乘2）决定要不要顺便刷专家，能带尽量带，否则之后要单独打  
> 最后是 `专家+力量`，单独打是因为Mod会大幅降低爬的效率，放最后是因为前面可能顺便打了
...
`受击缓冲`的容量不再是固定的3，而是随楼层变化，第一层为1，每上一层减0.1
...
```js
    /*
        ...
        变量frame是当前帧数，60帧=1秒
        “MOD_”是简写，在原代码中均为“this.S.setoptions.zenith_”
    */
