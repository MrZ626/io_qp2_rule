...
此模式的背景题材为爬塔，最终分数称为`高度`（Altitude）
...
`推进器等级`决定了高度增加量的倍率，最开始倍率为1，每升一级倍率增加×0.25（达到×2.75时变成白色不再看得出区别，实际无上限）
...
| - | - |
| 发送攻击 | 每行 1 点 |
| 抵消垃圾行（非专家模式） | 每行 0.5 点 |
| 消行（非专家模式） | 一行 1 点，两行及以上 2 点 |
| 单次落块若因以上方式获得了经验 | 额外 0.05 点 |

推进器经验每升级所需经验为 `4*当前等级`。

当经验大于需求点数时触发升级并扣除需求点数，同时推进器经验会 `锁定5秒` 不流失（流失规则见下文）

### 跳级

每次升级时，若经验仍然超过升级所需经验，则额外升 `经验/所需经验` 级，**且不扣除经验**

会出现 第 N 帧发出大spike，N+1 帧跳级，N+2 帧还升级的情况（比较复杂，待验证）

### 经验流失

推进器经验会随时间流失，每帧减少 `e*当前等级*(当前等级+1)/3600` 点。
...
每次降级时，前文提到的推进器经验锁定5秒会减少1秒，反复降级升级会更挣扎，不过至少保留1秒。重新达到高一级所需升级经验的一半时重置到5秒

以下列出一些算好的数据方便查阅：

| 级别 | 升级所需经验 | 降级所需秒数 | 每秒流失速率 |
| - | - | - | - |
| 1 | 4 | 40.00 | 0.1 |
| 2 | 8 | 26.67 | 0.3 |
| 3 | 12 | 20.00 | 0.6 |
...
```js
    // 一些常数表
    FloorDistan - [0, 50, 150, 300, 450, 650, 850, 1100, 1350, 1650, 1 / 0];
    TargetingGra - [0, 4.8, 3.9, 2.1, 1.4, 1.3, .9, .6, .4, .3, .2];
